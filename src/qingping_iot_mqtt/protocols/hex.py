...
from typing import Iterable, Optional, Final, Mapping
...
class HexSensorReadingMessage(SensorReadingsContainer):
  ...
  def decode_sensor_data_group(data: bytes, start: int = 4, length: int=6) -> Iterable[SensorReading]:
    ...
    1) SPEC: The air pressure is magnified 100 times, and the unit is kPa." 
      -> data stored as raw is in 0.1hPa = daPa; reasonable range is decided as >8000 daPa (800hPa)
    ...
    2) SPEC: Maybe CO2 depends on product
      -> data stored as raw is in ppm; reasonable range is decided as <8000 ppm
      -> CGP22C states range 400-9999 ppm, 9999 is reported as max value, but all values up to 9999 can be easily generated by user
    """
    if third_sensor_raw<8000: # too low to be pressure = must be CO2
      # FIXME: this can still be 9999 ppm CO2, I only know that PROP_DEVICE_PRODUCTID=0x0033 means CO2 sensor but it's in separate TLV
      # TODO: maybe let's store both readings and let higher-level logic decide which to pick?
    ...
    else: # too high to be CO2 = must be pressure
    ...
    for entry in message.frame.payload.entries:
      if entry.key in [
        ...
        for entry_index in range(entry_count):
          ...
          readings_context = SensorReadingsContext(readings=sensor_readings, origin=SensorReadingType.HISTORICAL, status=SensorReadingStatus.NORMAL, timestamp=timestamp+entry_index*interval)
        ...
        event_id = int.from_bytes(entry.value[0:0], byteorder="little")
        event_value = int.from_bytes(entry.value[1:4], byteorder="little")
        raise NotImplementedError(f"Event alert parsing not implemented yet, but found event id={event_id}, event_value={event_value}")
        pass
      elif entry.key in [
        ...
        timestamp = HexSensorReadingMessage.decode_timestamp(entry.value)
        sensor_data = int.from_bytes(entry.value[4:14], byteorder="little")
        event_setting_value = int.from_bytes(entry.value[15:16], byteorder="little")
        raise NotImplementedError(f"Event-based sensor reading parsing not implemented yet, but found TLV={entry.key} ts={timestamp}, sensor_data={sensor_data}, event_setting_value={event_setting_value}")
        ...
        readings_context = SensorReadingsContext(readings=realtime_readings, origin=SensorReadingType.REALTIME, status=SensorReadingStatus.NORMAL, timestamp=timestamp)
        self.readings.append(readings_context)
      else:
        pass
